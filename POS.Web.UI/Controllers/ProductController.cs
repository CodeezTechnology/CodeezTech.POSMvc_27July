...
namespace POS.Web.UI.Controllers
{
    public class ProductController : MasterController
    {
        ...
        public ActionResult Create(POS_PRODUCT POS_PRODUCT)
        {
            try
            {
                if (Session[SessionVariables.Session_UserInfo] != null)
                {
                    POS_PRODUCT.CREATEDBY = SessionHandling.UserInformation.USERNAME;
                    POS_PRODUCT.CREATEDWHEN = DateTime.Now;
                    if (ModelState.IsValid)
                    {
                        objNotify = _objBALProduct.Create(POS_PRODUCT);
